...
2.  **Simulation First**: All execution defaults to **DRY_RUN** (mock execution). Progression through **PAPER** (broker-simulated) and **LIVE** (real-money) requires explicit operator authorization, valid risk-limit checks, and an ARMED kill-switch.
3.  **Historical Isolation**: The **BACKTEST** mode is strictly isolated from all live execution data to preserve backtest fidelity and ledger integrity.
